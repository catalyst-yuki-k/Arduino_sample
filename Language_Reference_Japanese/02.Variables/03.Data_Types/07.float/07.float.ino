...
// Floating-point numbers are often used to approximate analog 
// and continuous values because they have greater resolution than integers. 
...
// Unlike other platforms, where you can get more precision 
// by using a double (e.g. up to 15 digits), on the Arduino, double is the same size as float.
// 
// Floating point numbers are not exact, and may yield strange results when compared. 
// For example 9.0 / 0.3 may not quite equal 30.0. 
...
// so should be avoided if, for example, a loop has to run at top speed for a critical timing function. 
