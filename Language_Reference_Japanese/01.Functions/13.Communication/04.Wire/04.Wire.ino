...
// making it consistent with other read/write libraries. 
// Because of this, send() and receive() have been replaced with read() and write().
// 
// Recent versions of the Wire library can use timeouts to prevent a lockup in the face of certain problems on the bus, 
// but this is not enabled by default (yet) in current versions. 
// It is recommended to always enable these timeouts when using the Wire library. 
// See the Wire.setWireTimeout function for more details.
// 
