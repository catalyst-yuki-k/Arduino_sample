...
// in microcontroller programs and can help solve timing problems. 
...
// because the program would need to constantly poll the sensor lines for the encoder, 
// in order to catch pulses when they occurred. 
// Other sensors have a similar interface dynamic too, 
// such as trying to read a sound sensor that is trying to catch a click, 
// or an infrared slot sensor (photo-interrupter) trying to catch a coin drop. 
// In all of these situations, using an interrupt can free the microcontroller 
// to get some other work done while not missing the input.
// 
...
// Nothing
// Interrupt Numbers
// Normally you should use digitalPinToInterrupt(pin), 
// rather than place an interrupt number directly into your sketch. 
...
// For historical reasons, this numbering does not always correspond directly 
// to the interrupt numbering on the ATmega chip (e.g. int.0 corresponds to INT4 on the ATmega2560 chip).
// 
